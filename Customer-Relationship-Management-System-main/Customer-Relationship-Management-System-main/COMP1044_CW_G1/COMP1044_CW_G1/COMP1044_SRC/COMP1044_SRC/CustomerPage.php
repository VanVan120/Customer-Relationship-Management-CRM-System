...
if ($roleTitle === "Admin") {
  // Admin can see all customers and their assigned staff
  $sql = "SELECT customer.*, staff.First_Name AS Staff_First_Name, staff.Last_Name AS Staff_Last_Name 
          FROM customer 
          LEFT JOIN staff ON customer.Staff_ID = staff.Staff_ID 
          ORDER BY Customer_ID ASC";
  $result = $conn->query($sql);
} else {
  // Sales representatives can only see their own customers
  $sql = "SELECT * FROM customer WHERE Staff_ID = ? ORDER BY Customer_ID ASC";
...
<head>
  <meta charset="UTF-8" />
  <title>ABB Robotics CRM - Customers</title>
...
<body>
  ...
  <div class="main-content">
    ...
    <header>
      <div class="search-wrapper">
        ...
        <form method="get" action="Search.php">
          ...
          <input type="hidden" name="source" value="CustomerPage">
        </form>
      </div>
    ...
    <main>
      ...
      <div class="error-message" style="background-color: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <strong>Error:</strong> Cannot delete this customer because it has related interactions or reminders. Delete those records first.
      </div>
      <?php endif; ?>
      <?php if (isset($_GET['error']) && $_GET['error'] === 'HasRelationships'): ?>
      <div class="error-message" style="background-color: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <strong>Error:</strong> Cannot delete this customer because it has related records. Please remove those records first.
      </div>
      <?php endif; ?>
      <?php if (isset($_GET['error']) && $_GET['error'] === 'CannotDelete'): ?>
      <div class="error-message" style="background-color: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <strong>Error:</strong> An unexpected error occurred while trying to delete this customer. Please try again later.
      </div>
      <?php endif; ?>
      
      <!-- Add these success messages -->
      ...
      <div class="success-message" style="background-color: #d4edda; color: #155724; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        <strong>Success:</strong> Customer information updated successfully.
      </div>
      <?php endif; ?>
      ...
      <div class="dashboard-header">
        ...
        <button onclick="window.location.href='manageEntity.php?type=customer'">Add New</button>
      </div>
          ...
          <tbody>
              ...
              ?>
                  <tr>
                      ...
                      <?php endif; ?>
                      <td>
                          <button class="btn-edit" onclick="window.location.href='manageEntity.php?type=customer&action=edit&id=<?php echo $row[$primaryKey]; ?>'">Edit</button>
                          <button class="btn-delete" onclick="if(confirm('Are you sure you want to delete this record?')) window.location.href='manageEntity.php?type=customer&action=delete&id=<?php echo $row[$primaryKey]; ?>'">Delete</button>
                      </td>
                  </tr>
              <?php
                  endwhile;
